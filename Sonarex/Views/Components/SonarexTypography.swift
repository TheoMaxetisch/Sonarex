import SwiftUI

enum SonarexTypography {
    static let screenTitle = Font.system(size: 25, weight: .bold)
    static let sheetTitle = Font.system(size: 22, weight: .bold)
    static let sectionTitle = Font.system(size: 17, weight: .bold)
    static let action = Font.system(size: 24, weight: .semibold)
    static let body = Font.system(size: 16, weight: .regular)
    static let bodyEmphasis = Font.system(size: 16, weight: .semibold)
    static let secondary = Font.system(size: 14, weight: .regular)
    static let secondaryEmphasis = Font.system(size: 14, weight: .semibold)
    static let trackTitle = Font.system(size: 14, weight: .semibold)
    static let trackArtist = Font.system(size: 12, weight: .regular)
    static let metadata = Font.system(size: 12, weight: .semibold)
    static let heroTitle = Font.system(size: 32, weight: .bold, design: .rounded)
    static let heroSubtitle = Font.system(size: 15, weight: .semibold)
    static let artworkSymbol = Font.system(size: 36, weight: .semibold)
    static let largeArtworkSymbol = Font.system(size: 66, weight: .semibold)
}
